...
> `NEEDS_EXEC_CHECK` — validate WAL archive + restore on a non-production Astro environment before relying on this procedure for production RPO.
...
{syn: RPO-targets-practitioner-judgment — RPO/RTO values below are design starting points; actual targets must be defined by business and compliance requirements, not by this reference file.}
...
> RPO targets in this table are design starting points; define actual RPO/RTO by business and compliance requirements.
...
| Risk | Mitigation |
|---|---|
| Restoring DB to a point ahead of the scheduler's last state | Restart the scheduler after restoring; state mismatch between DB and running tasks can cause zombie tasks {syn: B3-practitioner-judgment} |
