...
> **Note**: default values for `metadataPoolSize` and `resultBackendPoolSize` ("~10" and "~5") are cited from search summaries; verify against the current Astronomer Helm chart before production configuration. Flagged `NEEDS_EXEC_CHECK`.
...
| Large / high throughput | High worker count or KubernetesExecutor burst | Add PgBouncer replicas if single instance is the bottleneck; monitor connection saturation [B-S2] |
...
Citation marker → row mapping: B1→researcher B1, B2→researcher B2, B3→researcher B3, B4→researcher B4, B5→researcher B5, B6→researcher B6, B7→researcher B7, B8→researcher B8, A1-1→researcher A1-1, A1-2→researcher A1-2, A1-3→researcher A1-3, A1-4→researcher A1-4, B-S1→researcher B-S1, B-S2→researcher B-S2

## Gaps noticed
- Exact default values for `metadataPoolSize` and `resultBackendPoolSize` should be verified against the current Helm chart version; the researcher flagged this as not independently confirmed.

## Size check
Draft reference file: ~3,500 characters. Within target.
