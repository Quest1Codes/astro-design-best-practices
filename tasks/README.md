...
| SETUP-4 | Seed Source List | PARTIAL | — | Done informally for Clusters 1-2 only, as part of generating 001-020 (not run as its own gated task); Clusters 3-19 were generated without a separate seed-source-list gate either — each file cites its own sources inline, but the SETUP-4 deliverable itself was never produced |
| SETUP-5 | Tier-3 Partner Material Ingestion Path | NOT STARTED | — | |
| SETUP-6 | Skill Scaffold | MERGED (informally) | — | `SKILL.md` + all 19 `reference/` cluster subdirectories now exist on disk (126 files); SKILL.md's own Reference Files index was out of sync with this until it was regenerated to list all 19 clusters — see SKILL.md changelog context |
...
| 014-020 | Cluster 2 — Executor & Worker Architecture, original 7 topics | CRITIC-REVIEWED, NOT SIGNED OFF | — | Same process as above. This batch's Critic pass found 2 outright `FAIL`s (a fabricated KEDA polling-interval/cool-down figure, and two Kubernetes resource-limit claims cited to a source supporting neither) — both fixed; see `research/topics-011-020-critic-report.md`. The KEDA `cooldownPeriod` `NEEDS_EXEC_CHECK` was resolved against current Astronomer docs (300s/5-min is correct — see `keda-based-autoscaling.md`). This cluster's executor-decision and hybrid-strategy files originally predated the Astro executor becoming the Airflow 3.x default; both were rewritten in a later session to cover it, and `CeleryKubernetesExecutor` was corrected from "recommended" to "not even a selectable Astro Deployment executor." |
| 021-024 | Remaining Cluster 2 topics (SSHOperator legacy access, worker fleet capacity planning, zombie-task detection, GPU/specialized-compute workers) | SHIPPED, NOT CRITIC-REVIEWED, NOT SIGNED OFF | — | Written in a later session (not the original 4-stage pipeline run) — each file is sourced with real Astronomer Docs/Apache Airflow citations and cross-checked against sibling files for consistency, but did not go through a separate fresh-context Critic pass. Cluster 2 is now 11 of 11. |
| 025-100 | Clusters 3-13 | CRITIC-REVIEWED (later session), NOT SIGNED OFF | — | Originally shipped with each file's own `## Sources` section but no separate fresh-context Critic pass on record. A later session ran an adapted Critic pass (checking unsourced claims, `{syn:}` logic validity, cross-file contradictions, and papered-over gaps, using each file's own Sources as ground truth in place of a separate fact-sheet) across all of Clusters 3-19 and fixed the real problems it found — see the session's Critic-pass findings for specifics (not yet written to a `research/` critic-report file in this repo). |
| 101-130 | Cluster 14-19 topic tasks | CRITIC-REVIEWED (later session), NOT SIGNED OFF | — | Added after researching real AutoSys documentation (Broadcom TechDocs) — see the per-cluster grounding notes above. Same later-session Critic pass as 025-100 covered these too. |
